...
async function initDashboard() {
  try {
    ...
    // Check authentication first (if auth module is available)
    if (window.checkAuth) {
      const authenticated = await window.checkAuth();
      if (!authenticated) {
        console.log('User not authenticated, showing login prompt');
        const weekList = document.getElementById('eoyr-week-list');
        if (weekList) {
          weekList.innerHTML = `
            <div class="eoyr-loading">
              <h2 style="color: var(--eoyr-neon-green); margin-bottom: 1rem;">Please Log In</h2>
              <p style="color: #ccc; margin-bottom: 1.5rem;">You need to log in with GitHub to view your commit reports.</p>
              <button onclick="window.redirectToLogin(); return false;" class="eoyr-filter-button" style="font-size: 1.1rem; padding: 0.75rem 2rem;">
                Login with GitHub
              </button>
            </div>
          `;
    ...
    // Check if user has selected repos
    if (!repos || repos.length === 0) {
      const weekList = document.getElementById('eoyr-week-list');
      if (weekList) {
        weekList.innerHTML = `
          <div class="eoyr-loading">
            <h2 style="color: var(--eoyr-neon-green); margin-bottom: 1rem;">No Repositories Selected</h2>
            <p style="color: #ccc; margin-bottom: 1.5rem;">Please select repositories in settings to view your commit reports.</p>
            <button onclick="openRepoSettings(); return false;" class="eoyr-filter-button" style="font-size: 1.1rem; padding: 0.75rem 2rem;">
              Repository Settings
            </button>
          </div>
        `;
